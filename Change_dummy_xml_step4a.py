...
# =============================================================================
# insert text '         JOINT_DOF_OUTPUT_LIST="ALL" into xmlFiles
# opens xmlFiles to readlines
# enumerates over lines to find index i of word matching '<TIME_HISTORY_MB'
# than opens xmlFiles as write to insert the inserttext using 'slicing'method
# =============================================================================
     
    inserttext = ['         JOINT_DOF_OUTPUT_LIST="ALL"\n']
    ...
    for j in range(0, len(xmlFiles)):
        with open(xmlFiles[j], 'r') as f:
            ...
            for i, word in enumerate(lines):
                if '<TIME_HISTORY_MB' in word:
                    k=i +1                
                else: continue
        f.close()
        with open(xmlFiles[j], 'w') as f:
            xmlFiles[j] = lines[0:k] + inserttext + lines[k:]
            print(*lines[:], sep="\n")
