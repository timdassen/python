...
# =============================================================================
# # create a new list called 'harnessFiles'
# # define matches of strings in RAD file names
# # if their is a match, than append file to harnessFiles list
# =============================================================================    
    
    harnessFiles = []
    count1 =0
    matches = ["UP1805_include_05_harness","0000.rad"]
    for i in range(0, len(itemdigit1)):
        for file in os.listdir(itemdigit1[i]):
            if all(x in file for x in matches):
                harnessFiles.append(os.path.join(itemdigit1[i],file))
                count1+=1
            else:
                continue
    # print(harnessFiles)
...
# =============================================================================
# opens harnessFiles to readlines
# than opens again as write to write lines except for "/END"
# than opens as append to append "UP1805_include_05_harness_extra_card.txt"
# than writes the updated "UP1805_include_03_seat" rad file 
# =============================================================================

    for j in range(10, len(harnessFiles)):
        with open(harnessFiles[j], 'r') as f:
            lines = f.readlines()
        with open(harnessFiles[j], "w") as f:
            for line in lines:
                if line.strip("\n") != "/END":
                    f.write(line)
        with open(harnessFiles[j], "a+") as f:
            with open("UP1805_include_05_harness_extra_card.txt","r") as h:
